...
function detectColumnType(headerTextRaw) {
    const headerText = headerTextRaw.trim().toLowerCase();
    if (headerText.includes('mã ck'))
        return 'stock';
    if (headerText.includes('ngày tạo'))
        return 'date';
    if (headerText.includes('khối lượng'))
        return 'volume';
    if (headerText.includes('tên báo cáo'))
        return 'report';
    return null;
}
/**
...
const RandomData = {
    ...
    reportName: (stockOverride) => {
        const stock = stockOverride !== undefined && stockOverride !== null && stockOverride !== ''
            ? stockOverride
...
function isPotentialColumn(node) {
    if (node.type !== 'FRAME')
        return false;
...
function collectColumnsFromSelection(selection) {
    ...
    for (const node of selection) {
        if (isPotentialColumn(node))
            addColumn(node);
        if ('findAll' in node) {
            const nodeWithFindAll = node;
            const found = nodeWithFindAll.findAll((n) => isPotentialColumn(n));
...
figma.ui.onmessage = async (msg) => {
    if (msg.type === 'fill-smart-data') {
        ...
        const columnInfos = columns
            ...
            .filter((x) => x.dataType !== null);
        const stockByRowIndex = new Map();
        ...
        for (const { col, dataType } of columnInfos.filter((x) => x.dataType !== 'stock')) {
            ...
            for (let i = 1; i < children.length; i++) {
                ...
                await loadFontsForTextNode(cellValueText);
                if (dataType === 'date')
                    cellValueText.characters = RandomData.date();
                else if (dataType === 'volume')
                    cellValueText.characters = RandomData.volume();
